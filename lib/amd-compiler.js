...
function addDepFile(file, resInfo, options, fis) {
    if (isExternalModule(options.externals, resInfo.rawId)) {
        return;
    }

    var dynamicMods = options.dynamic;
    ...
    if (resInfo && resInfo.fullPath) {
        ...
        if (!info || !info.file) {
            if (!dynamicMods || dynamicMods.indexOf(resInfo.rawId) === -1) {
                fis.log.warn(
                    'cannot find require resource file: %s in %s',
                    resInfo.rawId, file.subpath
                );
            }
            return;
        }
 ...
 * 判断是否是外部模块
 *
 * @param {?Array.<string>} externals 外部模块 id
 * @param {string} id 要判断的 id
 * @return {boolean}
 */
function isExternalModule(externals, id) {
    return externals && externals.some(function (item) {
 ...
 * @param {Object} fis fis 实例
 */
function addFileDepInfo(file, info, moduleConfig, options, fis) {
 ...
 * @return {string}
 */
function parseModule(content, file, moduleConfig, options, fis) {
    var moduleAst;
    try {
        moduleAst = amdHelper.getAst(content);
    }
    catch (ex) {
        fis.log.warn('parse %s error: %s', file.subpath, ex);
    }

    var moduleInfos = moduleAst && amdHelper.parseAMDModule(moduleAst, true);
    if (!moduleInfos) {
        return content;
    }
 ...
 * 初始化模块配置
 *
 * @param {Object} opts 模块选项
 * @param {Object} fis fis 实例
 * @return {Object}
 */
exports.initModuleConfig = function (opts, fis) {
    var moduleConfig = amdHelper.initModuleConfig(opts.config, fis.project.getProjectPath());
    this.moduleConfig = moduleConfig;
    moduleConfig.extNames = opts.extNames || ['.js', '.vue'];
    this.fis = fis;
...
exports.compile = function (fileInfo, options) {
    ...
    if (!parsed && !file.isInline) {
        content = parseModule(undoCodeInfo.content, file, moduleConf, options, fis);
    }
