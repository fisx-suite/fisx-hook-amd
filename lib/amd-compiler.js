...
function parseModule(content, file, moduleConfig, fis) {
    ...
    moduleInfos.forEach(function (module) {
        addDepInfo(module.syncDeps, true);
        addDepInfo(module.asynDeps, false);
    });
...
exports.compile = function (fileInfo, options) {
    ...
    // 由于传入的文件信息可能是 html 文件，但处理的是其包含的脚本内容，
    // 因此不能直接通过 file.id 来判定要不要忽略
    if (file.ignoreAMD || (file.isJsLike && options.ignore(file.id))) {
        return;
    }
    ...
    var result = undoFisLangCode(content, fis);

    // 解析模块代码
    content = parseModule(result.content, file, this.moduleConfig, fis);

    // 恢复中间代码
    fileInfo.content = restoreFISLangCode(content, result.langInfo, fis);
};
